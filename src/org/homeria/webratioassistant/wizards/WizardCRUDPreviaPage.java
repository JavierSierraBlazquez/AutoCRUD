...
public class WizardCRUDPreviaPage extends WizardPage {
	...
	public void createControl(Composite parent) {
		...
		// FillLayout groupArbolLayout = new
		// FillLayout(org.eclipse.swt.SWT.VERTICAL);
		...
		// Necesito dividir la pantalla en: -SiteView o Area y dentro de cada
		// uno de ellos: nombre y un boton de a�adir
		arbol.addListener(SWT.Selection, new Listener() {
			...
			public void handleEvent(Event event) {
				...
				// a�adir un hijo - Se ha puesto como SIMPLE el arbol asi que
				// solo podr� seleccionarse de uno en uno
		...
		// GROUP
		// ALTA________________________________________________________________

		// Parte para a�adir elementos al arbol
		...
		// Se a�ade el input para meter el nombre y el boton de alta
		Button buttonAniadir = new Button(groupIntroducirTexto, SWT.PUSH);
		buttonAniadir.setText("A�adir");
		...
		buttonAniadir.addSelectionListener(new SelectionAdapter() {
			public void widgetSelected(SelectionEvent e) {
				...
				if (buttonRadioSTV.getSelection()) {
					...
					// Validaciones: el nombre del Area no puede ser igual a
					// otro ya existente.
					if (validacionesInsertarSiteViewArbol()) {
						...
						// Quedar marcado como seleccionado el nuevo elemento
						// a�adido
						TreeItem[] arrSelection = arbol.getSelection();
						itemSeleccionado = arrSelection;
						// a�adir un hijo - Se ha puesto como SIMPLE el arbol
						// asi que solo podr� seleccionarse de uno en uno
						buttonRadioArea.setEnabled(Boolean.TRUE);
				...
				// Area se crea dentro del elemento seleccionado
					...
					// Validaciones: el nombre del Area no puede ser igual a
					// otro ya existente.
					// Validaciones: se debe haber seleccionado previamente un
					// padre "itemSeleccionado"
					if (validacionesInsertarAreaArbol()) {
						...
						// Insertar tb en la lista que se corresponde con el
						// mapa
						...
						nuevoObjetoStViewArea.setTipo("AREA");// sera depende
																// del tipo
																// seleccionado
																// Area o
																// StvView
						...
						// Fin: Quedar marcado como seleccionado el nuevo
						// elemento a�adido
		...
		buttonDeshacer.addSelectionListener(new SelectionAdapter() {
			public void widgetSelected(SelectionEvent e) {
				// inicializa la lista de arboles y carga el arbol por primera
				// vez
				inicializarListaYarbol();
	...
	/**
	 * 
	 * Nombre: buscarElementoCoincideTextoRecursivo Funcion:
	 * 
	 * @return
	 */
	...
	/**
	 * 
	 * Nombre: validacionesInsertarSiteViewArbol Funcion:
	 * 
	 * @return
	 */
	private Boolean validacionesInsertarSiteViewArbol() {
		...
		if (null == nombre || (nombre != null && nombre.getText().isEmpty())) {
			// sacar mensaje por pantalla avisando de que se debe insertar un
			// nombre
	...
	/**
	 * 
	 * Nombre: validacionesInsertarArbol Funcion:
	 * 
	 * @return
	 */
